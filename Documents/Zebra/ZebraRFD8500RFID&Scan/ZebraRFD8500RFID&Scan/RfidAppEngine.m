...
- (void)initializeRfidSdkWithAppSettings
{
   ...
   // [m_RfidSdkApi srfidUnsubsribeForEvents:SRFID_EVENT_MASK_STATUS_OPERENDSUMMARY];
}
...
/*
    srfidTagData *tmp = nil;
    for (int i = 1; i < 50; i++)
    {
        tmp = [[srfidTagData alloc] init];
        [tmp setTagId:[[tagData getTagId] stringByReplacingOccurrencesOfString:@"0000" withString:[NSString stringWithFormat:@"%4d", i]]];
        [m_RadioOperationEngine eventTagData:tmp];
        [tmp release];
        
    }
*/    
    //[m_InventoryData addInventoryItem:tagData];
    
    
    //[self updateReadEventList];
}
...
- (void)srfidEventStatusNotify:(int)readerID aEvent:(SRFID_EVENT_STATUS)event aNotification:(id)notificationData
{
    ...
    if (event == SRFID_EVENT_STATUS_OPERATION_END_SUMMARY)
    {
    ...
    [m_RadioOperationEngine eventRadioOperation:(event == SRFID_EVENT_STATUS_OPERATION_BATCHMODE || event == SRFID_EVENT_STATUS_OPERATION_START)?YES:NO];
